...
gz_node = Node()
CAMERA_TOPIC = "/front_camera"  # ← UPDATE THIS
...
#     """Return True if a red object is visible in the frame."""
#     hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

#     # Red has two ranges on the hue wheel (it wraps around 0/180)
#     mask1 = cv2.inRange(hsv, np.array([0, 100, 100]), np.array([10, 255, 255]))
#     mask2 = cv2.inRange(hsv, np.array([170, 100, 100]), np.array([180, 255, 255]))
#     mask = mask1 | mask2

#     # Find contours — outlines of red blobs
#     contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

#     if not contours:
#         return False

#     # Check if the largest red blob is big enough to not be noise
#     largest = max(contours, key=cv2.contourArea)
#     return cv2.contourArea(largest) > 50
...
def detect_red(frame):
    ...
    # DEBUG: how many red pixels did the mask find?
    red_pixels = cv2.countNonZero(mask)
    print(f"    [DEBUG] Red pixels found: {red_pixels}")
...
def main():
    ...
    while time.time() - yaw_start < 60:
        ...
        # if latest_frame is not None:
           # if detect_red(latest_frame.copy()):
           #     print(f"  🔴 OBJECT DETECTED! Heading: {current}°")
           # else:
           #     print(f"  Scanning... Heading: {current}°  No target.")
        
        if latest_frame is not None:
            print(f"  [DEBUG] Frame received: {latest_frame.shape}")
        else:
            print(f"  [DEBUG] No frame yet — camera not connected?")

        if latest_frame is not None and not hasattr(main, '_saved'):
            cv2.imwrite("/home/paperspace/debug_frame.png", latest_frame)
            print(f"  [DEBUG] Saved frame to debug_frame.png")
            main._saved = True
